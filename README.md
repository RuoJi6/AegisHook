...
内置目录提供 26 条规则，覆盖基础约束、系统命令、数据库和 HTTP 请求；数据外传管道规则默认关闭。规则页可搜索名称和命令、筛选启停状态、使用滑动开关，并展开查看完整检测条件、样例和识别范围。具体覆盖及旧版本升级行为见 [规则目录说明](docs/rules.md)。
...
「限制漏洞确认后的批量取数」默认关闭。开关在当前提示词中插入或移除带起止标记的 R7 规则块，保存后生效；恢复默认会关闭。升级时仅自动替换与 v0.1.1 默认内容完全一致的旧 R7 规则块，并保存配置版本与审计；其他自定义内容和已关闭状态保留。新版 R7 模板仅根据当前参数限制通过已确认漏洞进行的取证：同一目标、同一漏洞，本次调用获取业务记录最多 10 条、敏感文件最多 5 个，每个最多读取 500 MB 的必要片段；一个调用包含循环或多个请求时合计本次读取量。明确限量且未超本次额度时不命中 R7，不因使用循环或漏洞已成立就拒绝；明确无界或超额取数时拒绝；正常下载网页、JS、CSS 等前端资源及状态检查不计入额度。模型不接收历史，无法判断历史验证状态或跨调用累计量；旧提示词中的累计额度也不能凭空推算。额度由提示词指导模型判断，尚无后端计数器。该限制仅用于进入模型审查的调用，前置允许规则仍可能直接放行。

模型审查只发送本次 `toolName`、`argumentsObj` 和 `hitlMode`，不发送历史命令、历史执行结果、用户会话或历史审批结论。服务端通过统一白名单约束所有 Agent 和 OpenAI / Anthropic 接口，旧客户端上报的 `userMessage` / `context` 也不会进入模型。Pi 不再采集最近 6 条工具执行记录，仅保留最近一条用户消息用于本机审计和人工查看；重新加载扩展后生效，服务端的当前调用限制无需等待客户端升级。已保存的背景和历史裁决不改写，新模型审查记录的 `modelContext` 为空字符串。已有自定义提示词保留，服务端在实际请求中追加仅审查当前调用的边界。
